...
int main() {
    ...
    assert(at(Screen::Settings,Editor::None,216,233,321).id==MenuRow+(int)MenuItem::Done);
    int personalMax=(int)PersonalItem::Count*72-288;
    assert(at(Screen::Personalize,Editor::None,personalMax,233,321).id==PersonalRow+(int)PersonalItem::Back);
    ...
    // Long buttons ignore hold duration; page cancellation consumes the release.
    ...
    assert(p.end(320,409)==Done);
    p.begin(t.id,t.bounds,310,398,100,false); p.cancel(); assert(p.end(310,398)==None);
}
